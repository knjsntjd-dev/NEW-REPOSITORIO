...
/* external_id: identificador próprio do visitante, sem dados
   pessoais. Segue encriptado em SHA-256, como o Meta exige para
   este campo. Sem pelo menos uma chave destas, o Meta não usa o
   evento para atribuição nem para otimização. */
$eid = texto($dados, 'external_id', 100) ?: (string)($_COOKIE['_eid'] ?? '');
if ($eid !== '') {
    $user_data['external_id'] = [hash('sha256', strtolower($eid))];
}
